 ...
 *
 * IMPORTANT: fixtures.home_team_id and away_team_id are NOT NULL in the
 * database schema (0001_schema.sql). This means we CANNOT insert a fixture
 * row until BOTH teams are known. For knockout fixtures where one or both
 * teams are still placeholders (e.g. "W74", "1A"), we skip writing that
 * fixture entirely — it will be created automatically on a later sync once
 * openfootball resolves the placeholder to a real team name.
 */
...
// openfootball uses labels like "1A", "2B", "Winner A", "W74", "L101" for
// knockout fixtures before the previous round is complete. These must NEVER
// be stored as real teams in the teams table.
const PLACEHOLDER_RE = /^(\d[A-Z]|Winner\b|Runner-up\b|Loser\b|[A-Z]\d+|W\d+|L\d+)/i;
...
async function syncFromOpenfootball() {
  ...
  // 3. Ensure each real team exists
  ...
  // 5. Upsert fixtures
  //
  // CRITICAL: home_team_id and away_team_id are NOT NULL in the schema.
  // We can ONLY write a fixture row once BOTH teams are resolved to real
  // UUIDs. If either side is still a placeholder (or a real name we can't
  // find in the teams table), we skip that fixture entirely this sync —
  // it gets created automatically the moment openfootball resolves it.
  ...
  for (const m of matches) {
    ...
    // Skip entirely — cannot satisfy NOT NULL constraint yet
    if (t1IsPlaceholder || t2IsPlaceholder) {
      stats.placeholdersSkipped++;
      continue;
    }

    const homeId = nameToId.get(t1) ?? null;
    const awayId = nameToId.get(t2) ?? null;

    if (!homeId) {
      stats.warnings.push(`Unknown team name "${t1}" — skipping fixture (no UUID found)`);
      continue;
    }
    if (!awayId) {
      stats.warnings.push(`Unknown team name "${t2}" — skipping fixture (no UUID found)`);
      continue;
    }
    ...
    const { error: fe } = await client
      .from("fixtures")
      .upsert(fixture, { onConflict: "external_id", ignoreDuplicates: false });
  ...
  // 6. Clean up any orphaned placeholder teams from older script versions
  try {
...
app.listen(PORT, async () => {
  ...
  console.log(
    "Initial sync:",
    r.error ?? `${r.fixturesUpserted} fixtures, ${r.scoresUpdated} scored, ${r.placeholdersSkipped} placeholder slots skipped`
  );
  ...
  setInterval(async () => {
    ...
    else console.log(`[${ts}] Auto-sync: ${r.fixturesUpserted} fixtures, ${r.scoresUpdated} scored, ${r.placeholdersSkipped} placeholder slots skipped`);
  }, 15 * 60 * 1000);
});